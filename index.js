...
subtotal.innerText = (price * quantity).toLocaleString();
shippingPriceElement.innerText = shippingPrice.toLocaleString();
total.innerText = (price * quantity + shippingPrice).toLocaleString();
...
const updateState = (number) => {
  ...
  // Order management
  if (quantity >= orderLimit && number > 0) {
  ...
  total.innerText = (subTotal + shippingPrice).toLocaleString();
};

incrementButton.addEventListener("click", () => updateState(1));
decrementButton.addEventListener("click", () => updateState(-1));
